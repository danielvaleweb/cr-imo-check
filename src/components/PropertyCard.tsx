...
const PropertyCard: React.FC<PropertyCardProps> = ({ prop, onClick, isFavorite, onToggleFavorite }) => {
  return (
    <motion.div 
      ...
      onClick={onClick}
      className="bg-white rounded-xl overflow-hidden shadow-sm border border-marromescuro/5 cursor-pointer transition-shadow duration-300 hover:shadow-xl"
    >
      ...
      <div className="p-6 space-y-6 relative">
        ...
        <div className="flex items-center justify-between pt-6 border-t border-marromescuro/10">
          ...
          >
            <Heart 
              className={`w-6 h-6 transition-all ${isFavorite ? 'fill-[#435B45] text-[#435B45]' : 'text-marromescuro/20'}`} 
            />
          </button>
        </div>
      </div>
